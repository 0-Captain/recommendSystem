...
from tensorflow.keras.layers import Dense, Embedding, Concatenate, Input, Reshape, Dropout, Flatten, Dot, BatchNormalization, Add, LSTM, Activation
...
def userModel():
    ...
    usersGenderModel = Dense(denseDimension, activation="relu", use_bias=True,
                             kernel_regularizer=regularizers.l2(0.001))(usersGenderModel)
    ...
    usersAgeModel = Dense(denseDimension, activation="relu", use_bias=True, kernel_regularizer=regularizers.l2(0.001))(
        usersAgeModel)
    ...
    usersJobIdModel = Dense(denseDimension, activation="relu", use_bias=True,
                            kernel_regularizer=regularizers.l2(0.001))(usersJobIdModel)
...
def movieModel():
    # ------------movie部分
    global moviesGenresInput
    moviesGenresInput = Input(shape=(moviesGenresInputDim,), dtype="float32", name='movieGenres')
    moviesGenresModel = Dense(16, activation='relu', use_bias=True, kernel_regularizer=regularizers.l2(0.001))(
        moviesGenresInput)
    ...
    moviesTitleModel = Dense(16, activation='relu', use_bias=True, kernel_regularizer=regularizers.l2(0.001))(
        moviesTitleModel)
...
def FM():
    userModel = Flatten()(user)
    userModel = Dense(32, activation='relu', use_bias=True, kernel_regularizer=regularizers.l2(0.00))(userModel)

    moviieModel = Flatten()(movie)
    moviieModel = Dense(32, activation='relu', use_bias=True, kernel_regularizer=regularizers.l2(0.00))(moviieModel)
    model = Dot(1)([userModel, moviieModel])
    return model
...
model = DeepFM()
history = model.fit([usersGender, usersAge, usersJobId, moviesTitle, moviesGenres], userRatings, batch_size=256, epochs=8, verbose=1, validation_split=0.2)
